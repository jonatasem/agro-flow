// Importa o React e o hook useState para gerenciar o formulário interno de fechamento do setor.
import React, { useState } from "react";

// Importa o hook personalizado de autenticação para validar as permissões de exibição.
import { useAuth } from "../../../hooks/useAuth";

// Importa o serviço HTTP contendo as rotas de Ordens de Serviço e setores.
import { workOrderService, type WorkOrder } from "../services/workOrderService";

// Importa a função utilitária para tratamento e extração de mensagens de erro amigáveis.
import { getErrorMessage } from "../../../utility/getErrorMessage";

// Define as propriedades estendidas aceitas pelo componente de cartão.
interface WorkOrderCardProps {
  order: WorkOrder;
  onEdit: () => void;   
  onDelete: () => void; 
  onRefresh: () => void; // 🚀 Prop adicionada para recarregar o dashboard após iniciar/finalizar serviços.
}

export const WorkOrderCard: React.FC<WorkOrderCardProps> = ({ order, onEdit, onDelete, onRefresh }) => {
  // Coleta dados do usuário logado para validações visuais e de propriedade de atendimento.
  const { user } = useAuth();
  
  // Estados para controlar o andamento das requisições e armazenamento temporário de inputs técnicos.
  const [loadingAction, setLoadingAction] = useState<string | null>(null);
  const [solucao, setSolucao] = useState("");
  const [causa, setCausa] = useState("");

  // Função utilitária para formatar datas e horas no padrão brasileiro (DD/MM/AAAA às HH:MM).
  ...
  // Função utilitária que analisa o status e injeta classes dinâmicas de estilo nos Badges.
  ...
  // 🚀 NOVO: Trata o clique do botão para assumir e iniciar o reparo do setor.
  const handleStartRepair = async (sectorServiceId: string) => {
    try {
      ...
      onRefresh(); // Atualiza a tela imediatamente.
    } catch (err: unknown) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-4 shadow-xl hover:border-slate-700/80 transition-colors">
      ...
      {/* Seção superior do cabeçalho do cartão */}
      <div className="flex justify-between items-start gap-4 border-b border-slate-800/80 pb-3">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Bloqueia botões gerenciais de edição e exclusão para cargos comuns */}
          {user?.role !== "TECNICO" && (
      ...
      {/* Seção central: Mapeia os setores vinculados a esta OS */}
      <div className="space-y-3">
        {order.setores?.map((sector) => (
          <div key={sector.id} className="bg-slate-950/60 p-3 rounded-xl border border-slate-800/50 space-y-2">
            ...
            {/* Renderiza histórico caso o setor já tenha sido resolvido */}
            ...
            {/* 🚀 BOTÕES DE OPERAÇÃO EM TEMPO REAL PARA OS SETORES */}
            {sector.status === "AGUARDANDO_MANUTENCAO" && (
              <button
                type="button"
                disabled={loadingAction !== null}
                onClick={() => handleStartRepair(sector.id)}
                className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 font-bold text-xs rounded-xl transition-all disabled:opacity-50"
              >
            ...
            {sector.status === "EM_MANUTENCAO" && (
              <form onSubmit={(e) => handleFinishRepair(e, sector.id)} className="space-y-2 pt-1 border-t border-slate-900">
      ...
      {order.operator && (
        <div className="text-[11px] text-slate-500 pt-1 flex justify-between items-center">
          ...
          {/* Renderiza o carimbo da última modificação se houver dados de alteração. */}
          {order.updatedAt && (
