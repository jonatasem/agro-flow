...
export class GetWorkOrderByIdController {
  async handle(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!workOrderId) {
      return reply.status(400).send({ error: "O ID da ordem é obrigatório." });
    }

    const getWorkOrderByIdService = new GetWorkOrderByIdService();

    try {
      const workOrder = await getWorkOrderByIdService.execute({
        workOrderId,
        userRole,
      });
      ...
      const statusCode =
        error.message === "Ordem de Serviço não encontrada." ? 404 : 400;
